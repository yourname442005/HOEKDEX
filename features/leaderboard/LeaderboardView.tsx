    ...
    <div className="space-y-6 pb-24 lg:pb-12 max-w-5xl mx-auto">
      ...
      <PageHeader
        ...
        actions={
          <Link
            href="/friends"
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#FFFDF9] hover:bg-[#F5EFE6] border border-[#E7E0D8] text-xs font-bold text-stone-700 transition-colors shadow-xs"
          >
      ...
      {currentUserEntry && (
        <div className="p-4 rounded-2xl bg-gradient-to-r from-rose-500/10 via-[#FFFDF9] to-[#FFFDF9] border border-rose-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-rose-600 text-white flex items-center justify-center font-black text-sm shadow-md shadow-rose-600/20">
          ...
          <div className="flex items-center gap-1.5 text-xs font-extrabold text-rose-600 bg-rose-500/15 px-3 py-1.5 rounded-full border border-rose-500/25 self-start sm:self-auto">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span className={cn('text-xs font-extrabold', isActive ? 'text-rose-600' : 'text-[#1C1917]')}>
                  {p.label}
                </span>
                {isActive && <Flame className="w-3.5 h-3.5 text-amber-600" />}
              </div>
      ...
      {currentRankedList.length > 0 && (
        <div className="p-6 sm:p-8 rounded-3xl bg-[#FFFDF9] border border-[#E7E0D8] space-y-6 shadow-xs relative overflow-hidden">
          <div className="text-center space-y-1">
            <span className="text-caption text-rose-600 block">Social Game Standings</span>
            <h3 className="text-h2 text-[#1C1917]">Podium Leaders</h3>
          </div>

          <div className="grid grid-cols-3 gap-2 sm:gap-4 items-end pt-4 max-w-2xl mx-auto">
            {/* 2ND PLACE (Left) */}
            {top2 ? (
              <div className="flex flex-col items-center text-center space-y-2 p-3 sm:p-4 rounded-2xl bg-stone-50 border border-stone-300 relative shadow-xs">
                <div className="w-8 h-8 rounded-full bg-stone-200 border-2 border-stone-400 flex items-center justify-center text-stone-700 font-black text-xs absolute -top-4 shadow-xs">
                  <Medal className="w-4 h-4 text-stone-600" />
                </div>
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full overflow-hidden border-2 border-stone-300 mt-2">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={top2.avatarUrl} alt={top2.displayName} className="w-full h-full object-cover" />
                </div>
                <div>
                  <p className="text-xs sm:text-sm font-extrabold text-[#1C1917] truncate max-w-[100px]">
                    {top2.displayName} {top2.userId === user.id && '(You)'}
                  </p>
                  <p className="text-[11px] font-black text-stone-600">{top2.totalXp.toLocaleString()} XP</p>
                </div>
                <span className="px-2.5 py-0.5 rounded-full bg-stone-200 text-[10px] font-black text-stone-700">
                  2ND
                </span>
              </div>
            ) : <div />}
            ...
            {top1 ? (
              <div className="flex flex-col items-center text-center space-y-2 p-4 sm:p-5 rounded-3xl bg-gradient-to-b from-[#FFFDF9] via-[#FFFBF0] to-amber-500/10 border-2 border-amber-400 relative shadow-md shadow-amber-500/10 -translate-y-2">
                <div className="w-10 h-10 rounded-full bg-amber-500/20 border-2 border-amber-500 flex items-center justify-center text-amber-600 font-black text-sm absolute -top-5 shadow-md">
                ...
                <div>
                  <p className="text-sm sm:text-base font-black text-[#1C1917] truncate max-w-[120px]">
                    {top1.displayName} {top1.userId === user.id && '(You)'}
                  </p>
                  <p className="text-xs font-black text-amber-600">{top1.totalXp.toLocaleString()} XP</p>
                </div>
            ...
            {/* 3RD PLACE (Right) */}
            {top3 ? (
              <div className="flex flex-col items-center text-center space-y-2 p-3 sm:p-4 rounded-2xl bg-amber-900/5 border border-amber-700/30 relative shadow-xs">
                <div className="w-8 h-8 rounded-full bg-amber-700/20 border-2 border-amber-700 flex items-center justify-center text-amber-800 font-black text-xs absolute -top-4 shadow-xs">
                ...
                <div>
                  ...
                  <p className="text-[11px] font-black text-amber-800">{top3.totalXp.toLocaleString()} XP</p>
                </div>
                <span className="px-2.5 py-0.5 rounded-full bg-amber-700/20 text-[10px] font-black text-amber-900">
                  3RD
                </span>
              </div>
            ) : <div />}
      ...
      <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 flex items-start gap-3 text-xs shadow-xs">
        ...
        <div className="space-y-0.5">
          <p className="font-bold text-[#1C1917]">Strict Privacy Boundary</p>
